...
var fs = require("fs");//file system모듈 가져오기

//http모듈로부터 server객체 생성하기
var server = http.createServer(function(request, response){//요청과 응답

//아래의 코드들이 동작하기 전에 서버는 클라이언트가 원하는게 무엇인지 구분을 먼저 해야한다
//요청분석
//요청 분석을 들어가려면, 요청 정보를 담고 있는 request객체를 이용해야 한다
...
fs.readFile("./2회원폼유효성체크.html","utf-8",function(error,data){
    if(error){
        ...
        //http프로토콜상, 약속을 지켜서, 즉, 형식을 지켜서 보내자
        response.writeHead(200,{"Content-type":"text/html;charset=utf-8"});
        response.end(data);//클라이언트에게 컨텐츠 전송, 회원가입 양식을..
        
    }
});
//2)가입을 원할경우에 db에 넣어줘야 하고,

//3)이미지를 원하면 이미지를 보여줘야 하고
...
server.listen(7979,function(){
    console.log("Server is running 7979 port....")
});
